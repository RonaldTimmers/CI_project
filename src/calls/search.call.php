...
if(!empty($_SERVER['HTTP_X_REQUESTED_WITH']) &&  strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') 
{
    ...
    $secondQuery = isset($_GET['secondQuery']) ? test_input($_GET['secondQuery']) : NULL;
    ...
    */
    ...
    $objectThumbs = ThumbsController::initThumbs( $pagetype, $searchKeywords, $secondQuery,  null, false, $options );
    $objectThumbs->setThumbData()->getThumbData();
    
    /*
    $searchQL = new SearchQL;

    $searchThumbs['thumbs'] = $searchQL     ->setQueryArguments( $keywords, $shops, $options )                                                    
                                            ->buildQuery( $pagetype, $secondQuery )
                                            ->setSearchMeta()
                                            ->setSearchCategories( $db->linode ) 
                                            ->setSearchResults( $db->linode )
                                            ->getSearchResults();
    */
    /*
     * Max Matches isset because to keep results below 2000 for performance
     */
    ...
    $searchThumbs['searchCategories']   = $objectThumbs->searchQL->getSearchCategories() ;
